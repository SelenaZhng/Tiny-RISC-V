...
#-------------------------------------------------------------------------
# SingleCoreSys_noparam
#-------------------------------------------------------------------------

class SingleCoreSys_noparam( Component ):
  id_ = 0
  ...
  def __init__( s, *args, **kwargs ):
    ...
    s.ffi.cdef("""
      ...
      } VSingleCoreSys_noparam_t;

      VSingleCoreSys_noparam_t * VSingleCoreSys_noparam_create_model( const char * );
      void VSingleCoreSys_noparam_destroy_model( VSingleCoreSys_noparam_t *);
      void VSingleCoreSys_noparam_comb_eval( VSingleCoreSys_noparam_t * );
      void VSingleCoreSys_noparam_seq_eval( VSingleCoreSys_noparam_t * );
      void VSingleCoreSys_noparam_assert_on( VSingleCoreSys_noparam_t *, bool );
      bool VSingleCoreSys_noparam_has_assert_fired( VSingleCoreSys_noparam_t * );
      void VSingleCoreSys_noparam_line_trace( VSingleCoreSys_noparam_t *, char * );

    """)

    # Print the modification time stamp of the shared lib
    # print('Modification time of {}: {}'.format(
    #       'libSingleCoreSys_noparam_v.so', os.path.getmtime( './libSingleCoreSys_noparam_v.so' ) ))
    ...
    s._ffi_inst = s.ffi.dlopen('./libSingleCoreSys_noparam_v.so', s.ffi.RTLD_NODELETE | s.ffi.RTLD_NOW)

    # increment instance count
    SingleCoreSys_noparam.id_ += 1
  ...
  def finalize( s ):
    ...
    s._ffi_inst.VSingleCoreSys_noparam_destroy_model( s._ffi_m )
  ...
  def construct( s, *args, **kwargs ):
    ...
    if int(s._ip_cfg.vl_trace):
      if bool(s._ip_cfg.vl_trace_filename):
        ...
        verilator_vcd_file = "SingleCoreSys_noparam.verilator1.vcd"
    ...
    s._ffi_m = s._ffi_inst.VSingleCoreSys_noparam_create_model( ffi_vl_vcd_file )
    ...
    _ffi_m = s._ffi_m
    _ffi_inst_comb_eval = s._ffi_inst.VSingleCoreSys_noparam_comb_eval
    _ffi_inst_seq_eval  = s._ffi_inst.VSingleCoreSys_noparam_seq_eval

    # declare the port interface
    s.commit_inst = OutPort( Bits1 )
    s.dcache_access = OutPort( Bits1 )
    s.dcache_miss = OutPort( Bits1 )
    s.icache_access = OutPort( Bits1 )
    s.icache_miss = OutPort( Bits1 )
    ...
    def seq_upblk():
      ...
      if s._ffi_inst.VSingleCoreSys_noparam_has_assert_fired( _ffi_m ):
        raise AssertionError("A Verilog assertion fired in the Verilator simulation!")

  def assert_on( s, enable ):
    assert isinstance( enable, bool )
    s._ffi_inst.VSingleCoreSys_noparam_assert_on( s._ffi_m, enable )

  def line_trace( s ):
    if 1:
      s._ffi_inst.VSingleCoreSys_noparam_line_trace( s._ffi_m, s._ffi_m._cffi_line_trace_str )
      return s._convert_string( s._ffi_m._cffi_line_trace_str ).decode('ascii')
    else:
      return f' clk={s.clk}, commit_inst={s.commit_inst}, dcache_access={s.dcache_access}, dcache_miss={s.dcache_miss}, icache_access={s.icache_access}, icache_miss={s.icache_miss}, reset={s.reset}, stats_en={s.stats_en}, dmem.reqstream.msg={s.dmem.reqstream.msg}, dmem.reqstream.rdy={s.dmem.reqstream.rdy}, dmem.reqstream.val={s.dmem.reqstream.val}, dmem.respstream.msg={s.dmem.respstream.msg}, dmem.respstream.rdy={s.dmem.respstream.rdy}, dmem.respstream.val={s.dmem.respstream.val}, imem.reqstream.msg={s.imem.reqstream.msg}, imem.reqstream.rdy={s.imem.reqstream.rdy}, imem.reqstream.val={s.imem.reqstream.val}, imem.respstream.msg={s.imem.respstream.msg}, imem.respstream.rdy={s.imem.respstream.rdy}, imem.respstream.val={s.imem.respstream.val}, mngr2proc.msg={s.mngr2proc.msg}, mngr2proc.rdy={s.mngr2proc.rdy}, mngr2proc.val={s.mngr2proc.val}, proc2mngr.msg={s.proc2mngr.msg}, proc2mngr.rdy={s.proc2mngr.rdy}, proc2mngr.val={s.proc2mngr.val},'

...
#include "obj_dir_MultiCoreSys_noparam/VMultiCoreSys_noparam.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_MultiCoreSys_noparam/VMultiCoreSys_noparam__Syms.h"
#include "svdpi.h"
#endif
...
VMultiCoreSys_noparam_t * VMultiCoreSys_noparam_create_model( const char *vcd_filename ) {

  VMultiCoreSys_noparam_t  * m;
  VMultiCoreSys_noparam * model;
  ...
  m     = new VMultiCoreSys_noparam_t;
  model = new VMultiCoreSys_noparam(context_ptr);
  ...
  for ( int i_0 = 0; i_0 < 4; i_0++ )
    ...
    m->mngr2proc_msg[i_0] = &model->mngr2proc_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->mngr2proc_rdy[i_0] = &model->mngr2proc_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->mngr2proc_val[i_0] = &model->mngr2proc_val[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2mngr_msg[i_0] = &model->proc2mngr_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2mngr_rdy[i_0] = &model->proc2mngr_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2mngr_val[i_0] = &model->proc2mngr_val[i_0];
...
void VMultiCoreSys_noparam_destroy_model( VMultiCoreSys_noparam_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VMultiCoreSys_noparam * model = (VMultiCoreSys_noparam *) m->_cffi_model;
...
void VMultiCoreSys_noparam_comb_eval( VMultiCoreSys_noparam_t * m ) {
...
void VMultiCoreSys_noparam_seq_eval( VMultiCoreSys_noparam_t * m ) {
...
void VMultiCoreSys_noparam_assert_on( VMultiCoreSys_noparam_t * m, bool enable ) {
...
bool VMultiCoreSys_noparam_has_assert_fired( VMultiCoreSys_noparam_t * m ) {
...
#if VLINETRACE
void VMultiCoreSys_noparam_line_trace( VMultiCoreSys_noparam_t * m, char* str ) {
  ...
  // which is called `v`.
  // svSetScope( &model->__VlSymsp->__Vscope_MultiCoreSys_noparam__v );
  ...
  const svScope dut_scope = svGetScopeFromName("TOP.MultiCoreSys_noparam.v");
  assert( dut_scope );
  svSetScope( dut_scope );

  VMultiCoreSys_noparam::line_trace( words );

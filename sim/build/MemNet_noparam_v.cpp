...
#include "obj_dir_MemNet_noparam/VMemNet_noparam.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_MemNet_noparam/VMemNet_noparam__Syms.h"
#include "svdpi.h"
#endif
...
VMemNet_noparam_t * VMemNet_noparam_create_model( const char *vcd_filename ) {

  VMemNet_noparam_t  * m;
  VMemNet_noparam * model;
  ...
  m     = new VMemNet_noparam_t;
  model = new VMemNet_noparam(context_ptr);
  ...
  for ( int i_0 = 0; i_0 < 4; i_0++ )
    ...
    m->cache2net_reqstream_msg[i_0] = model->cache2net_reqstream_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->cache2net_reqstream_rdy[i_0] = &model->cache2net_reqstream_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->cache2net_reqstream_val[i_0] = &model->cache2net_reqstream_val[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->cache2net_respstream_msg[i_0] = model->cache2net_respstream_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->cache2net_respstream_rdy[i_0] = &model->cache2net_respstream_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->cache2net_respstream_val[i_0] = &model->cache2net_respstream_val[i_0];

  m->net2mem_reqstream_msg = model->net2mem_reqstream_msg;
  m->net2mem_reqstream_rdy = &model->net2mem_reqstream_rdy;
  m->net2mem_reqstream_val = &model->net2mem_reqstream_val;
  m->net2mem_respstream_msg = model->net2mem_respstream_msg;
  m->net2mem_respstream_rdy = &model->net2mem_respstream_rdy;
  m->net2mem_respstream_val = &model->net2mem_respstream_val;
...
void VMemNet_noparam_destroy_model( VMemNet_noparam_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VMemNet_noparam * model = (VMemNet_noparam *) m->_cffi_model;
...
void VMemNet_noparam_comb_eval( VMemNet_noparam_t * m ) {
...
void VMemNet_noparam_seq_eval( VMemNet_noparam_t * m ) {
...
void VMemNet_noparam_assert_on( VMemNet_noparam_t * m, bool enable ) {
...
bool VMemNet_noparam_has_assert_fired( VMemNet_noparam_t * m ) {
...
#if VLINETRACE
void VMemNet_noparam_line_trace( VMemNet_noparam_t * m, char* str ) {
  ...
  // which is called `v`.
  // svSetScope( &model->__VlSymsp->__Vscope_MemNet_noparam__v );
  ...
  const svScope dut_scope = svGetScopeFromName("TOP.MemNet_noparam.v");
  assert( dut_scope );
  svSetScope( dut_scope );

  VMemNet_noparam::line_trace( words );

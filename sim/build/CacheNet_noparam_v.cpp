...
#include "obj_dir_CacheNet_noparam/VCacheNet_noparam.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_CacheNet_noparam/VCacheNet_noparam__Syms.h"
#include "svdpi.h"
#endif
...
VCacheNet_noparam_t * VCacheNet_noparam_create_model( const char *vcd_filename ) {

  VCacheNet_noparam_t  * m;
  VCacheNet_noparam * model;
  ...
  m     = new VCacheNet_noparam_t;
  model = new VCacheNet_noparam(context_ptr);
  ...
  for ( int i_0 = 0; i_0 < 4; i_0++ )
    ...
    m->net2cache_reqstream_msg[i_0] = model->net2cache_reqstream_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->net2cache_reqstream_rdy[i_0] = &model->net2cache_reqstream_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->net2cache_reqstream_val[i_0] = &model->net2cache_reqstream_val[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->net2cache_respstream_msg[i_0] = &model->net2cache_respstream_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->net2cache_respstream_rdy[i_0] = &model->net2cache_respstream_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->net2cache_respstream_val[i_0] = &model->net2cache_respstream_val[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2net_reqstream_msg[i_0] = model->proc2net_reqstream_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2net_reqstream_rdy[i_0] = &model->proc2net_reqstream_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2net_reqstream_val[i_0] = &model->proc2net_reqstream_val[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2net_respstream_msg[i_0] = &model->proc2net_respstream_msg[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2net_respstream_rdy[i_0] = &model->proc2net_respstream_rdy[i_0];

  for ( int i_0 = 0; i_0 < 4; i_0++ )

    m->proc2net_respstream_val[i_0] = &model->proc2net_respstream_val[i_0];
...
void VCacheNet_noparam_destroy_model( VCacheNet_noparam_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VCacheNet_noparam * model = (VCacheNet_noparam *) m->_cffi_model;
...
void VCacheNet_noparam_comb_eval( VCacheNet_noparam_t * m ) {
...
void VCacheNet_noparam_seq_eval( VCacheNet_noparam_t * m ) {
...
void VCacheNet_noparam_assert_on( VCacheNet_noparam_t * m, bool enable ) {
...
bool VCacheNet_noparam_has_assert_fired( VCacheNet_noparam_t * m ) {
...
#if VLINETRACE
void VCacheNet_noparam_line_trace( VCacheNet_noparam_t * m, char* str ) {
  ...
  // which is called `v`.
  // svSetScope( &model->__VlSymsp->__Vscope_CacheNet_noparam__v );
  ...
  const svScope dut_scope = svGetScopeFromName("TOP.CacheNet_noparam.v");
  assert( dut_scope );
  svSetScope( dut_scope );

  VCacheNet_noparam::line_trace( words );
